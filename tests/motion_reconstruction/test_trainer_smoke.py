...
def test_trainer_runs_one_epoch_and_saves_latest_checkpoint(tmp_path: Path):
    ...
    checkpoint = tmp_path / "runs" / "smoke" / "checkpoints" / "latest.pt"
    assert checkpoint.is_file()
    payload = torch.load(checkpoint, map_location="cpu")
    assert payload["epoch"] == 1
    assert payload["feature_schema"]["robot_feature_dim"] == 8
