...
@configclass
class G1FlatPPODualTokenRunnerCfg(G1FlatPPORunnerCfg):
    obs_groups = (
        {
            "policy": [
                "command_with_noise_wo_privilege",
    ...
    experiment_name = "g1_flat_dual_token"
...
class G1FlatCVAEDistillationStudentMultiTeacherCfg(RslRlDistillationRunnerCfg):
    num_steps_per_env = 24
    max_iterations = 90001
    obs_groups = (
        {
...
@configclass  # 对有特权信息训练的教师网络进行蒸馏
class G1FlatFSQCVAEDistillationStudentMultiTeacherCfg(RslRlDistillationRunnerCfg):
    num_steps_per_env = 24
    max_iterations = 90001
    obs_groups = (
        {
