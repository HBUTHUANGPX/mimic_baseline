...
class MDRXFlatPPORunnerCfg(RslRlOnPolicyRunnerCfg):
...
@configclass  # 特权信息的训练
class MDRXFlatPPOPureRunnerCfg(MDRXFlatPPORunnerCfg):
    obs_groups = (
        {
            "policy": [
                "command",
...
class MDRXFlatPPODistillSingleFSQRunnerCfg(MDRXFlatPPORunnerCfg):
    obs_groups = (
        {
            "policy": [
                "command_with_noise_wo_privilege",
...
class MDRXFlatPPOSingleFSQRunnerCfg(MDRXFlatPPORunnerCfg):
    def __post_init__(self):
        super().__post_init__()
...
@configclass
class MDRXFlatPPODualFSQRunnerCfg(MDRXFlatPPORunnerCfg):
    obs_groups = (
        {
            "policy": [
                "command_with_noise_wo_privilege",
    ...
    experiment_name = "mdrx_flat_dual_fsq"
...
@configclass
class MDRXFlatPPODualTokenRunnerCfg(MDRXFlatPPORunnerCfg):
    obs_groups = (
        {
            "policy": [
                "command_with_noise_wo_privilege",
    ...
    experiment_name = "mdrx_flat_dual_token"
    ...
    def __post_init__(self):
        ...
        self.policy.class_name = "ActorCriticDualToken"
        self.algorithm.class_name = "PPO"

@configclass  # 有特权信息WO DR 的训练
class MDRXFlatTeacherPPORunnerCfg(RslRlOnPolicyRunnerCfg):
    num_steps_per_env = 24
    max_iterations = 90001
    save_interval = 500
    obs_groups = (
    ...
    experiment_name = "mdrx_flat_teacher"
    policy = RslRlPpoActorCriticCfg(
        init_noise_std=0.6,
        actor_obs_normalization=True,
        critic_obs_normalization=True,
...
class PureMDRXFlatPPORunnerCfg(RslRlOnPolicyRunnerCfg):
    num_steps_per_env = 24
    max_iterations = 90001
    obs_groups = (
        {
    ...
    save_interval = 5000
    experiment_name = "pure_mdrx_flat"
    policy = RslRlPpoActorCriticCfg(
        init_noise_std=0.8,
        actor_obs_normalization=True,
        critic_obs_normalization=True,
...
class MDRXFlatCVAEDistillationStudentMultiTeacherCfg(RslRlDistillationRunnerCfg):
    num_steps_per_env = 24
    max_iterations = 90001
    obs_groups = (
        {
    ...
    save_interval = 500
    experiment_name = "mdrx_flat_distillation"
...
@configclass  # 对有特权信息训练的教师网络进行蒸馏
class MDRXFlatFSQCVAEDistillationStudentMultiTeacherCfg(RslRlDistillationRunnerCfg):
    num_steps_per_env = 24
    max_iterations = 90001
    obs_groups = (
        {
