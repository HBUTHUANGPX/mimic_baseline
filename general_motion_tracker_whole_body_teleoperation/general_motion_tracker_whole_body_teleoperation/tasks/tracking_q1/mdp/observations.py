...
def motion_ref_pos_b_window(
    ...
    """Return reference-body positions for the full temporal window.

    Each frame in the window is expressed relative to the current robot
    ...
    Returns:
        Flattened tensor with ordered frames `[t - n, ..., t, ..., t + m]`.
    """
    command = _get_command(env, command_name)
...
def motion_ref_ori_b_window(
    ...
    """Return reference-body orientations for the full temporal window.
    ...
    Returns:
        Flattened 6D rotation representation for the ordered temporal window.
    """
    command = _get_command(env, command_name)
...
def motion_body_pos_b_window(
    ...
    """Return motion body positions for the full temporal window.
    ...
    Returns:
        Flattened tensor containing body positions for all frames in time order.
    """
    command = _get_command(env, command_name)
    return command._flatten_window(command._motion_body_pos_b_window)


def motion_body_ori_b_window(
    env: ManagerBasedEnv, command_name: str
) -> torch.Tensor:
    """Return motion body orientations for the full temporal window.
    ...
    Returns:
        Flattened 6D rotation representation for all bodies and all frames.
    """
    command = _get_command(env, command_name)
    return command._flatten_window(command._motion_body_ori_b_mat_window[..., :2])


def joint_pos_delta_window(
    env: ManagerBasedEnv, command_name: str
) -> torch.Tensor:
    """Return joint-position deltas for the full temporal window.

    The delta is always computed against the current robot joint positions at
    the center frame.
    ...
    Returns:
        Flattened delta tensor in temporal order.
    """
    command = _get_command(env, command_name)
    return command._flatten_window(command._joint_pos_delta_window)


def motion_joint_pos_window(
    env: ManagerBasedEnv, command_name: str
) -> torch.Tensor:
    """Return raw motion joint positions for the full temporal window.
    ...
    Returns:
        Flattened joint-position tensor in temporal order.
    """
    command = _get_command(env, command_name)
    return command._flatten_window(command.joint_pos_window)


def motion_joint_vel_window(
    env: ManagerBasedEnv, command_name: str
) -> torch.Tensor:
    """Return raw motion joint velocities for the full temporal window.
    ...
    Returns:
        Flattened joint-velocity tensor in temporal order.
    """
    command = _get_command(env, command_name)
    return command._flatten_window(command.joint_vel_window)
