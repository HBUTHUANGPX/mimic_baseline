...
from tasks.tracking_q1.mdp.commands import MotionCommand
from general_motion_tracker_whole_body_teleoperation.tasks.tracking_q1.mdp.rewards import _get_body_indexes
...
def bad_ref_ori(env: ManagerBasedRLEnv, asset_cfg: SceneEntityCfg, command_name: str, threshold: float) -> torch.Tensor:
    ...
    command: MotionCommand = env.command_manager.get_term(command_name)
    motion_projected_gravity_b = math_utils.quat_apply_inverse(command.ref_quat_w, asset.data.GRAVITY_VEC_W)

    robot_projected_gravity_b = math_utils.quat_apply_inverse(command.robot_ref_quat_w, asset.data.GRAVITY_VEC_W)
