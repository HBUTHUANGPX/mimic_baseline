...
class simulator(infere):
    ...
    def _init_motion_window_offsets(self):
        ...
        with open(env_cfg_path, "r", encoding="utf-8") as file:
            # IsaacLab exports config files with python-specific YAML tags such
            # as ``!!python/tuple`` and ``python/object/apply:builtins.slice``.
            # Deployment only reads a trusted local artifact generated by the
            # training pipeline, so ``unsafe_load`` is required here to recover
            # the window metadata instead of failing during initialization.
            env_cfg = yaml.unsafe_load(file) or {}
