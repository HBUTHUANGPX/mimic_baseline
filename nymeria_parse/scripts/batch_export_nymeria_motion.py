...
from nymeria_parse.motion_export.batch import (  # noqa: E402
    DEFAULT_BATCH_OUTPUT_ROOT,
    DEFAULT_TEST_DATA_ROOT,
    BatchExportResult,
    discover_nymeria_sequence_tasks,
    export_nymeria_batch_annotation,
    export_nymeria_batch_smpl,
    export_nymeria_batch_soma_bvh,
)
from nymeria_parse.motion_export.soma_bvh import DEFAULT_SOMA_BATCH_SIZE  # noqa: E402
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Batch export Nymeria test_data motion assets. SMPL/annotation can use multiprocessing; SOMA BVH is sequential."
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if "annotation" in args.exports:
        results = export_nymeria_batch_annotation(
            tasks,
    ...
    if "smpl" in args.exports:
        results = export_nymeria_batch_smpl(
            tasks,
            workers=args.workers,
            start_frame=args.start_frame,
            end_frame=args.end_frame,
    ...
    if "soma-bvh" in args.exports:
        results = export_nymeria_batch_soma_bvh(
            tasks,
            workers=1,
            start_frame=args.start_frame,
            end_frame=args.end_frame,
