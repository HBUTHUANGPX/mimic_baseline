...
from ._compat import ensure_legacy_dependency_apis
...
class SMPLIdentityModel(BaseIdentityModel):
    ...
    def __init__(self, data_root, low_lod, device, model_type="smpl", **kwargs):
        ...
        if imt == "smpl" and model_path.suffix == ".npz":
            with np.load(model_path, allow_pickle=True) as data:
                data_struct = smplx.utils.Struct(**{k: data[k] for k in data.files})
            smpl_base_model = smplx.SMPL(
